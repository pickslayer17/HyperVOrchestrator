...
# Значения подменяет оркестратор из конфига; в гостя прокидываются через -ArgumentList.
...
Invoke-Command -VMName $vmName -Credential $cred -ArgumentList $vmIp, $prefix, $gateway, $dns, $proxyHostPort, $vmUser, $ifAlias -ScriptBlock {
    param($vmIp, $prefix, $gateway, $dns, $proxyHostPort, $vmUser, $ifAlias)
